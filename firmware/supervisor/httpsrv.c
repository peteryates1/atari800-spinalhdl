...
"function load(){$('#path').textContent=dir;"
" j('/api/list?dir='+encodeURIComponent(dir)).then(r=>r.json()).then(d=>{"
"  const u=$('#list');u.innerHTML='';"
"  if(dir!=='/'){const li=document.createElement('li');li.innerHTML=\"<span class='n d'>.. (up)</span>\";"
"   li.onclick=()=>{dir=dir.replace(/\\/[^/]*\\/?$/,'')||'/';load()};u.appendChild(li)}"
...
"   if(e.dir)nm.onclick=()=>{dir=(dir==='/'?'':dir)+'/'+e.name;load()};"
"   li.appendChild(nm);"
