...
class AnticDelay(cycle_length: Int = 16) extends Component {
  val io = new Bundle {
    ...
    // debug
    val dma_clock_out    = out Bool()
    val hcount_out       = out Bits(8 bits)
    val vcount_out       = out Bits(9 bits)
  }

  val cycle_length_bits = log2Up(cycle_length)
  ...
  // Line buffer (reg_file)
  val regFile1 = new RegFile(BYTES = 48, WIDTH = 6)
  ...
  val nmiShiftregReg  = Reg(Bits(16 bits)) init B(16 bits, default -> true)
  val nmiShiftregNext = Bits(16 bits)
  ...
  val hcountReg  = Reg(Bits(8 bits)) init B(0, 8 bits)
  val hcountNext = Bits(8 bits)
  ...
  val playfieldDmaStartCycle   = Bits(8 bits)
  val playfieldDmaEndCycle     = Bits(8 bits)
  ...
  val anPrevReg  = Reg(Bits(3 bits)) init B"000"
  val anPrevNext = Bits(3 bits)
  ...
  val colourClockCountReg  = Reg(Bits(cycle_length_bits bits)) init B(0, cycle_length_bits bits)
  val colourClockCountNext = Bits(cycle_length_bits bits)
  val colourClockCountRegTopThree = Bits(3 bits)
  ...
  nmistReg                    := nmistNext
  dmactlRawReg                := dmactlRawNext
  chactlReg                   := chactlNext
  vblankReg                   := vblankNext
  vsyncReg                    := vsyncNext
  pmbaseReg                   := pmbaseNext
  allowRealDmaReg             := allowRealDmaNext
  displayShiftReg             := displayShiftNext
  chbaseRawReg                := chbaseRawNext
  instructionReg              := instructionNext
  firstLineOfInstructionReg   := firstLineOfInstructionNext
  lastLineOfInstructionReg    := lastLineOfInstructionNext
  dmaSpeedReg                 := dmaSpeedNext
  instructionTypeReg          := instructionTypeNext
  instructionFinalRowReg      := instructionFinalRowNext
  shiftRateReg                := shiftRateNext
  shiftTwobitReg              := shiftTwobitNext
  twopixelReg                 := twopixelNext
  singleColourCharacterReg    := singleColourCharacterNext
  multiColourCharacterReg     := multiColourCharacterNext
  twolineCharacterReg         := twolineCharacterNext
  mapBackgroundReg            := mapBackgroundNext
  dliEnabledReg               := dliEnabledNext
  twoPartInstructionReg       := twoPartInstructionNext
  descendersReg               := descendersNext
  hscrolReg                   := hscrolNext
  vscrolRawReg                := vscrolRawNext
  vscrolEnabledReg            := vscrolEnabledNext
  vscrolLastEnabledReg        := vscrolLastEnabledNext
  hscrolEnabledReg            := hscrolEnabledNext
  shiftclockReg               := shiftclockNext
  hblankReg                   := hblankNext
  anReg                       := anNext
  anPrevReg                   := anPrevNext
  dmaFetchReg                 := dmaFetchNext
  dmaAddressReg               := dmaAddressNext
  dmaCacheReg                 := dmaCacheNext
  dmaCacheReadyReg            := dmaCacheReadyNext
  ...
  characterReg                := characterNext
  displayedCharacterReg       := displayedCharacterNext
  refreshPendingReg           := refreshPendingNext
  refreshFetchReg             := refreshFetchNext
  dliNmiReg                   := dliNmiNext
  vbiNmiReg                   := vbiNmiNext
  nmienRawReg                 := nmienRawNext
  delayDisplayShiftReg        := delayDisplayShiftNext
  nmiShiftregReg              := nmiShiftregNext
  penhReg                     := penhNext
  penvReg                     := penvNext
  colourClockCountReg         := colourClockCountNext

  // -----------------------------------------------------------------------
  // Colour clock generation
  // -----------------------------------------------------------------------
  colourClockCountRegTopThree := colourClockCountReg(cycle_length_bits - 1 downto cycle_length_bits - 3)

  colourClockHalfX := False
  colourClock1x    := False
  colourClock2x    := False
  colourClock4x    := False
  colourClock8x    := False

  colourClockCountNext := B(colourClockCountReg.asUInt + 1)

  when(io.ANTIC_ENABLE_179) {
    colourClockCountNext := B(1, cycle_length_bits bits)
  }

  colourClock8x := True

  when(colourClockCountReg(cycle_length_bits - 4 downto 0).orR === False) {
    switch(colourClockCountRegTopThree) {
      is(B"000") { colourClockHalfX := True; colourClock1x := True; colourClock2x := True; colourClock4x := True }
      is(B"001") { colourClock4x := True }
      is(B"010") { colourClock2x := True; colourClock4x := True }
      is(B"011") { colourClock4x := True }
      is(B"100") { colourClock1x := True; colourClock2x := True; colourClock4x := True }
      is(B"101") { colourClock4x := True }
      is(B"110") { colourClock2x := True; colourClock4x := True }
      is(B"111") { colourClock4x := True }
    }
  }
  ...
  switch(dmactlDelayedReg(6 downto 5)) {
    is(B"01") { dmactlDelayedEnabled := True }
    is(B"10") {
      enableDma                := colourClock1x
      colourClockSelected      := colourClock2x
      colourClockSelectedHighres := colourClock4x
      dmactlDelayedEnabled     := True
    }
    is(B"11") {
      enableDma                := colourClock2x
      colourClockSelected      := colourClock4x
      colourClockSelectedHighres := colourClock8x
      dmactlDelayedEnabled     := True
    }
    default {}
  }
  ...
  when(colourClock1x) {
    ...
    when(colourClockHalfX) { hcountNext(0) := True }
  }
  when(hcountReset) { hcountNext := B(0, 8 bits) }
  ...
  // -----------------------------------------------------------------------
  playfieldDmaStartCycle   := B(0xFF, 8 bits)
  playfieldDmaEndCycle     := B(0xFF, 8 bits)
  ...
  when(hscrolEnabledReg) {
    switch(dmactlDelayedReg(1 downto 0)) {
      is(B"10", B"11") {
        playfieldDmaStartCycle := B"0001" ## hscrolReg(3 downto 1) ## B"1"
        playfieldDmaEndCycle   := B"1101" ## hscrolReg(3 downto 1) ## B"0"
      }
      is(B"01") {
        playfieldDmaStartCycle := B"0010" ## hscrolReg(3 downto 1) ## B"1"
        playfieldDmaEndCycle   := B"1100" ## hscrolReg(3 downto 1) ## B"0"
      }
      default {}
    }
  } otherwise {
    switch(dmactlDelayedReg(1 downto 0)) {
      is(B"11") { playfieldDmaStartCycle := B"00010001"; playfieldDmaEndCycle := B"11010000" }
      is(B"10") { playfieldDmaStartCycle := B"00100001"; playfieldDmaEndCycle := B"11000000" }
      is(B"01") { playfieldDmaStartCycle := B"00110001"; playfieldDmaEndCycle := B"10110000" }
      default {}
    }
  }
  ...
  switch(dmactlDelayedReg(1 downto 0)) {
    is(B"11") {
      playfieldDisplayStartCycle := B"00101011"
      when(hscrolReg.asUInt > U"1100") { playfieldDisplayStartCycle := B"00101111" }
      playfieldDisplayEndCycle := B"11011101"
    }
    is(B"10") { playfieldDisplayStartCycle := B"00101111"; playfieldDisplayEndCycle := B"11001111" }
    is(B"01") { playfieldDisplayStartCycle := B"00111111"; playfieldDisplayEndCycle := B"10111111" }
    default {}
  }
  ...
  playfieldDmaStart           := False
  playfieldDmaEnd             := False
  playfieldDisplayActiveNext  := playfieldDisplayActiveReg
  dmaFetchRequest             := False
  dmaAddressNext              := dmaAddressReg
  ...
  lineBufferAddressNext       := B(0, 8 bits)
  loadLineBufferAddress       := False
  incrementLineBufferAddress  := False
  playfieldLoad               := False
  hblankNext                  := hblankReg
  hcountReset                 := False
  vcountIncrement             := False
  characterNext               := characterReg
  displayedCharacterNext      := displayedCharacterReg
  dliNmiNext                  := dliNmiReg
  vbiNmiNext                  := vbiNmiReg
  wsyncReset                  := False
  loadRefreshCount            := False
  refreshCountNext            := B(0, 4 bits)
  updateRowCount              := False
  vscrolLastEnabledNext       := vscrolLastEnabledReg

  when(colourClock1x) {
    // Playfield start/end
    when(hcountReg.asUInt === playfieldDmaStartCycle.asUInt) {
      playfieldDmaStart  := True
      loadLineBufferAddress := True
    }
    when(hcountReg.asUInt === playfieldDmaEndCycle.asUInt) {
      playfieldDmaEnd := True
    }
    when(hcountReg === playfieldDisplayStartCycle) {
      playfieldDisplayActiveNext := True
    }
    when(hcountReg === playfieldDisplayEndCycle) {
      playfieldDisplayActiveNext := False
    }

    when(dmaClockCharacterData) {
      when(dmactlDelayedEnabled && instructionTypeReg === MODE_CHARACTER) {
        dmaFetchRequest := True
      }
    ...
    switch(hcountReg) {
      is(B"00000000") { // missile DMA
        ...
        when(dmactlDelayedReg(4)) {
          dmaAddressNext := pmbaseReg(5 downto 1) ## B"011" ## vcountReg(7 downto 0)
        } otherwise {
          dmaAddressNext := pmbaseReg ## B"011" ## vcountReg(7 downto 1)
        }
      }
      is(B"00000010") { // display list dma
        firstLineOfInstructionNext := False
        when(instructionTypeReg === MODE_JVB) {
          firstLineOfInstructionNext := firstLineOfInstructionReg | vblankReg
        } otherwise {
          when(lastLineOfInstructionReg) {
            dmaFetchRequest := dmactlDelayedEnabled
            dmaAddressNext := displayListAddressReg
            dmaFetchDestinationNext := DMA_FETCH_INSTRUCTION
            firstLineOfInstructionNext := True
      ...
      is(B"00000100", B"00000110", B"00001000", B"00001010") { // player DMA
        dmaFetchRequest := dmactlDelayedReg(3)
        dmaFetchDestinationNext := DMA_FETCH_NULL
        when(dmactlDelayedReg(4)) {
          dmaAddressNext := pmbaseReg(5 downto 1) ## B(hcountReg(3 downto 1).asUInt + 2, 3 bits).asBits ## vcountReg(7 downto 0)
        } otherwise {
          dmaAddressNext := pmbaseReg ## B(hcountReg(3 downto 1).asUInt + 2, 3 bits).asBits ## vcountReg(7 downto 1)
        }
      }
      is(B"00001100") { // lms lower byte
        dmaFetchRequest := dmactlDelayedEnabled & firstLineOfInstructionReg & twoPartInstructionReg
        dmaAddressNext := displayListAddressReg
        dmaFetchDestinationNext := DMA_FETCH_LIST_LOW
      }
      is(B"00001110") { // lms upper byte
        dmaFetchRequest := dmactlDelayedEnabled & firstLineOfInstructionReg & twoPartInstructionReg
        dmaAddressNext := displayListAddressReg
        dmaFetchDestinationNext := DMA_FETCH_LIST_HIGH
      ...
      is(B"00100001") { hblankNext := False }
      is(B"00100100") { playfieldLoad := True }
      is(B"00110001") { loadRefreshCount := True; refreshCountNext := B(0, 4 bits) }
      is(B"11010010") { wsyncReset := True }
      is(B"11010100") {
        allowRealDmaNext := False
        loadRefreshCount := True
        refreshCountNext := B"1001"
      }
      is(B"11011000") { lastLineOfInstructionNext := lastLineOfInstructionLive }
      is(B"11011110") { vcountIncrement := True }
      is(B"11011101") { hblankNext := True }
      is(B"11100011") {
        hcountReset := True
        allowRealDmaNext := ~vblankReg
      }
      default {}
    }
    ...
    when(instructionTypeReg === MODE_CHARACTER && dmaClockCharacterInc) {
      characterNext := lineBufferDataOut
      incrementLineBufferAddress := True
    }
  ...
  refreshPendingNext    := refreshPendingReg
  refreshFetchNext      := refreshFetchReg

  when(colourClock1x && ~hcountReg(0)) {
    refreshFetchNext := False
    when(refreshPendingReg && (~dmaFetchNext || ~allowRealDmaNext)) {
      refreshFetchNext   := True
      refreshPendingNext := False
    }
    when(hcountReg(2 downto 1) === B"01" && refreshCountReg.asUInt < 9) {
      incrementRefreshCount := True
      refreshFetchNext   := ~dmaFetchNext
      refreshPendingNext := dmaFetchNext
    }
  }
  ...
  // NMI handling
  // -----------------------------------------------------------------------
  nmienDelay.io.syncReset := False
  nmienDelay.io.dataIn    := nmienRawReg
  nmienDelay.io.enable    := io.ANTIC_ENABLE_179
  nmienDelayedReg         := nmienDelay.io.dataOut

  nmiNext := ~((dliNmiReg & nmienDelayedReg(1)) | (vbiNmiReg & nmienDelayedReg(0)))
  ...
  anticDmaClock1.io.enableDma      := enableDma
  anticDmaClock1.io.playfieldStart := playfieldDmaStart
  anticDmaClock1.io.playfieldEnd   := playfieldDmaEnd
  anticDmaClock1.io.vblank         := vblankReg
  anticDmaClock1.io.slowDma        := slowDmaS
  anticDmaClock1.io.mediumDma      := mediumDmaS
  anticDmaClock1.io.fastDma        := fastDmaS
  dmaClockCharacterName := anticDmaClock1.io.dmaClockOut0
  dmaClockCharacterInc  := anticDmaClock1.io.dmaClockOut1
  dmaClockBitmapData    := anticDmaClock1.io.dmaClockOut2
  dmaClockCharacterData := anticDmaClock1.io.dmaClockOut3

  // Line buffer
  regFile1.io.ADDR    := lineBufferAddressReg(5 downto 0)
  regFile1.io.DATA_IN := lineBufferDataIn
  regFile1.io.WR_EN   := lineBufferWrite
  lineBufferDataOut   := regFile1.io.DATA_OUT

  // Vertical scrolling
  vscrolDelay.io.syncReset := False
  vscrolDelay.io.dataIn    := vscrolRawReg
  vscrolDelay.io.enable    := io.ANTIC_ENABLE_179
  vscrolDelayedReg         := vscrolDelay.io.dataOut

  loadRowCount       := False
  rowCountNext       := B(0, 4 bits)
  incrementRowCount  := False
  lastLineOfInstructionLive := False

  when(updateRowCount) {
    when(vscrolEnabledReg && ~vscrolLastEnabledReg) { rowCountNext := vscrolDelayedReg }
    when(firstLineOfInstructionReg) { loadRowCount := True }
    .otherwise                       { incrementRowCount := True }
  }
  ...
  chbaseDelay.io.syncReset := False
  chbaseDelay.io.dataIn    := chbaseRawReg
  chbaseDelay.io.enable    := io.ANTIC_ENABLE_179
  chbaseDelayedReg         := chbaseDelay.io.dataOut

  // -----------------------------------------------------------------------
  // Instruction decode
  // -----------------------------------------------------------------------
  dmaSpeedNext                := NO_DMA
  instructionTypeNext         := MODE_BLANK
  shiftRateNext               := SLOW_SHIFT
  shiftTwobitNext             := False
  twopixelNext                := False
  singleColourCharacterNext   := False
  multiColourCharacterNext    := False
  twolineCharacterNext        := False
  mapBackgroundNext           := False
  instructionFinalRowNext     := B(0, 4 bits)
  twoPartInstructionNext      := instructionReg(6)
  instructionBlankNext        := False
  vscrolEnabledNext           := instructionReg(5)
  hscrolEnabledNext           := instructionReg(4)
  dliEnabledNext              := instructionReg(7)
  descendersNext              := False
  forceFinalRow               := False

  switch(instructionReg(3 downto 0)) {
    is(B"0000") {
      instructionTypeNext := MODE_BLANK
      instructionFinalRowNext := False ## instructionReg(6 downto 4)
      twoPartInstructionNext := False
      instructionBlankNext := True
      vscrolEnabledNext := False
      hscrolEnabledNext := False
    }
    is(B"0001") {
      instructionTypeNext := MODE_JUMP
      instructionFinalRowNext := B(0, 4 bits)
      instructionBlankNext := True
      vscrolEnabledNext := False
      hscrolEnabledNext := False
      twoPartInstructionNext := True
      when(instructionReg(6)) { instructionTypeNext := MODE_JVB; forceFinalRow := True }
    }
  ...
  // DMA fetching / cache
  // -----------------------------------------------------------------------
  instructionNext               := instructionReg
  displayListAddressNextDma     := displayListAddressReg
  loadDisplayListAddressDma     := False
  memoryScanAddressNext         := memoryScanAddressReg
  loadMemoryScanAddress         := False
  incrementDisplayListAddress   := False
  loadDisplayShiftFromMemory    := False
  displayListAddressLowTempNext := displayListAddressLowTempReg
  lineBufferDataIn              := B(0, 8 bits)
  lineBufferWrite               := False
  dmaCacheNext                  := dmaCacheReg
  dmaCacheReadyNext             := dmaCacheReadyReg
  dmaFetchNext                  := dmaFetchRequest | dmaFetchReg

  when(dmaFetchReg && memoryReadyBoth) {
    dmaCacheNext      := io.MEMORY_DATA_IN
    dmaCacheReadyNext := True
    dmaFetchNext      := False
  }

  when(vblankReg && instructionTypeReg === MODE_JVB) {
    instructionNext := B(0, 8 bits)
  }

  when(dmaCacheReadyReg && ~hcountReg(0)) {
    switch(dmaFetchDestinationReg) {
      ...
      is(DMA_FETCH_NULL)        {}
      is(DMA_FETCH_INSTRUCTION) {
        instructionNext := dmaCacheReg
        incrementDisplayListAddress := True
      }
      is(DMA_FETCH_LIST_LOW) {
        when(instructionTypeReg === MODE_JUMP || instructionTypeReg === MODE_JVB) {
          displayListAddressLowTempNext := dmaCacheReg
          incrementDisplayListAddress := True
        } otherwise {
          incrementDisplayListAddress := True
          memoryScanAddressNext(7 downto 0) := dmaCacheReg
          loadMemoryScanAddress := True
        }
      }
      is(DMA_FETCH_LIST_HIGH) {
        when(instructionTypeReg === MODE_JUMP || instructionTypeReg === MODE_JVB) {
          displayListAddressNextDma := dmaCacheReg ## displayListAddressLowTempReg
          loadDisplayListAddressDma := True
        } otherwise {
          incrementDisplayListAddress := True
          memoryScanAddressNext(15 downto 8) := dmaCacheReg
          loadMemoryScanAddress := True
        }
      }
      default {}
  ...
  when(colourClockSelected) {
    ...
    switch(shiftRateReg) {
      is(SLOW_SHIFT) {
        enableShift := (shiftclockReg(0) & ~(hscrolReg(1) & hscrolEnabledReg)) | (shiftclockReg(2) & hscrolReg(1) & hscrolEnabledReg)
      }
  ...
  // -----------------------------------------------------------------------
  // AN output calculation
  // -----------------------------------------------------------------------
  anCurrent := B"000"
  dataLive  := B"00"

  when(shiftTwobitReg) {
    anCurrent(0) := delayDisplayShiftReg(0)
    anCurrent(1) := delayDisplayShiftReg(1)
    anCurrent(2) := True

    when(instructionTypeReg === MODE_CHARACTER) {
      dataLive(0) := delayDisplayShiftReg(0)
      dataLive(1) := delayDisplayShiftReg(1)

      when(descendersReg) {
        when(delayDisplayShiftReg(3 downto 2) === B"11") {
          when(rowCountReg.asUInt < 2) { dataLive := B"00" }
        } otherwise {
          when(rowCountReg.asUInt >= 8) { dataLive := B"00" }
        }
      }

      when(delayDisplayShiftReg(4)) {
        when(chactlReg(1)) {
          anCurrent(0) := ~(dataLive(0) & ~chactlReg(0))
          anCurrent(1) := ~(dataLive(1) & ~chactlReg(0))
        } otherwise {
          anCurrent(0) := dataLive(0) & ~chactlReg(0)
          anCurrent(1) := dataLive(1) & ~chactlReg(0)
        }
      } otherwise {
        anCurrent(0) := dataLive(0)
        anCurrent(1) := dataLive(1)
      }
    }

    when(multiColourCharacterReg) {
      switch(delayDisplayShiftReg(1 downto 0)) {
        is(B"00") { anCurrent := B"000" }
        is(B"01") { anCurrent := B"100" }
        is(B"10") { anCurrent := B"101" }
        is(B"11") { anCurrent := B"11" ## delayDisplayShiftReg(4) }
      }
    }
    ...
    when(mapBackgroundReg) {
      switch(delayDisplayShiftReg(1 downto 0)) {
        ...
        is(B"01") { anCurrent := B"100" }
        is(B"10") { anCurrent := B"101" }
        is(B"11") { anCurrent := B"110" }
      }
    }
  } otherwise {
    when(singleColourCharacterReg) {
      when(delayDisplayShiftReg(1)) {
        anCurrent(0) := delayDisplayShiftReg(3)
        anCurrent(1) := delayDisplayShiftReg(4)
        anCurrent(2) := True
      } otherwise {
        anCurrent := B"000"
      }
    ...
    when(mapBackgroundReg) {
      when(delayDisplayShiftReg(1)) { anCurrent := B"100" }
      .otherwise                      { anCurrent := B"000" }
    }
  }

  // AN prev
  anPrevNext := anPrevReg
  when(colourClockSelected) { anPrevNext := anCurrent }

  // AN next with blanking
  anNext := anReg
  when(colourClockSelected) {
    anNext := anCurrent
    when(hscrolReg(0) & hscrolEnabledReg) { anNext := anPrevReg }
    when(~playfieldDisplayActiveReg | instructionBlankReg | vblankReg | ~playfieldDmaEnabled) { anNext := B"000" }
    when(vblankReg | hblankReg) {
      anNext(0) := vsyncReg | twopixelReg
      anNext(1) := ~vsyncReg
  ...
  // Wsync delay
  // -----------------------------------------------------------------------
  wsyncDelay.io.syncReset := False
  wsyncDelay.io.dataIn    := wsyncWrite
  wsyncDelay.io.enable    := io.ANTIC_ENABLE_179
  wsyncDelayedWrite       := wsyncDelay.io.dataOut
  ...
  // Register writes
  // -----------------------------------------------------------------------
  nmienRawNext                := nmienRawReg
  dmactlRawNext               := dmactlRawReg
  chactlNext                  := chactlReg
  hscrolNext                  := hscrolReg
  vscrolRawNext               := vscrolRawReg
  chbaseRawNext               := chbaseRawReg
  pmbaseNext                  := pmbaseReg
  wsyncNext                   := (wsyncDelayedWrite | wsyncReg) & ~wsyncReset
  nmistReset                  := False
  wsyncWrite                  := False
  loadDisplayListAddressCpu   := False
  displayListAddressNextCpu   := displayListAddressReg
  ...
  when(addrDecoded(8))  { io.DATA_OUT := hcountReg }
  ...
  // NMI shift register
  nmiShiftregNext := nmiReg ## nmiShiftregReg(15 downto 1)

  // Light pen
  penvNext := penvReg
  penhNext := penhReg
  when(~io.lightpen) {
    penvNext := vcountReg(8 downto 1)
    penhNext := hcountReg
  }
  ...
  // Outputs
  // -----------------------------------------------------------------------
  io.NMI_N_OUT                 := nmiShiftregReg(0)
  io.dma_clock_out             := dmaClockCharacterName
  io.AN                        := anReg
