...
class SyncresetEnableDivider(COUNT: Int = 1, RESETCOUNT: Int = 0) extends Component {
  val io = new Bundle {
  ...
  val countReg      = Reg(UInt(WIDTH bits)) init U(0, WIDTH bits)
  ...
  when(io.syncreset) {
    countNext := U(RESETCOUNT, WIDTH bits)
  }
