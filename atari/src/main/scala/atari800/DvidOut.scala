...
class DvidOut extends Component {
  ...
  val pixelArea = new ClockingArea(pixelDomain) {
    ...
    // Latch TMDS words. pixel → tmds is a synchronous related-clock crossing
    // (tmds = 5× pixel); tag so the SpinalHDL CDC checker accepts it.
    val ch0Lat = RegNext(tmdsCh0) init 0
    val ch1Lat = RegNext(tmdsCh1) init 0
    val ch2Lat = RegNext(tmdsCh2) init 0
    ch0Lat.addTag(crossClockDomain)
    ch1Lat.addTag(crossClockDomain)
    ch2Lat.addTag(crossClockDomain)
  }
