...
class Atari800Ep4cgx150DualPllTop extends Component {
  ...
  val jopArea = new ClockingArea(sysDomain) {
    ...
    // SioBridge pins — same clock domain, no CDC needed
    val sioPins = cluster.devicePins("sioBridge")
    def sioPin[T <: Data](name: String): T =
      sioPins.elements.find(_._1 == name).get._2.asInstanceOf[T]
  ...
  val atariArea = new ClockingArea(atariDomain) {
    ...
    // SIO bridge: JOP monitors SIO bus and injects drive responses
    def sioPin[T <: Data](name: String): T = jopArea.sioPin[T](name)

    sioPin[Bool]("sioCommand")  := atariCore.io.SIO_COMMAND
    sioPin[Bool]("sioTxd")      := atariCore.io.SIO_TXD
    sioPin[Bool]("sioClockout") := atariCore.io.SIO_CLOCKOUT
    atariCore.io.SIO_RXD        := sioPin[Bool]("sioRxd")
...
object Atari800Ep4cgx150DualPllSv extends App {
  ...
  // Generate IoAddr.java with auto-allocated device addresses
  IoAddrGenerator.generate(JopCoreForAtariDualPll.config)
}

/** Generates IoAddr.java from the JOP device map.
 *  Emits per-device base + per-register constants so Java code never
 *  hardcodes I/O addresses. Runs the IoAddressAllocator deterministically
 *  to match the addresses assigned during SpinalHDL elaboration. */
object IoAddrGenerator {
  ...
  def generate(config: JopCoreConfig): Unit = {
    val devices = config.devices
    val bootName = DeviceTypes.bootDeviceName(devices)
    val descriptors = DeviceTypes.toDescriptorsForAllocation(devices, bootName)
    val allocated = IoAddressAllocator.allocate(descriptors)
    val addrMap = allocated.sortBy(_.baseAddr)
    ...
    for (ad <- addrMap) {
      ...
      val offset = ioOffset(ad.baseAddr)
      sb.append(s"\n\tpublic static final int ${devName} = Const.IO_BASE + ${hexOffset(offset)};")
      for ((subAddr, regName) <- ad.descriptor.registerNames) {
        val constName = s"${devName}_$regName"
        if (subAddr == 0)
          sb.append(s"\n\tpublic static final int $constName = $devName;")
        else
          sb.append(s"\n\tpublic static final int $constName = $devName + $subAddr;")
      }
      sb.append("\n")
    }

    // Interrupt number constants (matches JopCore interrupt wiring order)
    // Timer is always interrupt 0; I/O device interrupts start at 1.
    // allocated (not addrMap) preserves the allocator's return order which
    // matches JopCore's allocatedDevices.foreach iteration.
    sb.append("\n\t// Interrupt numbers")
    sb.append("\n\tpublic static final int INT_TIMER = 0;")
    var intIdx = 1
    for (ad <- allocated) {
      val ic = ad.descriptor.interruptCount
      if (ic > 0) {
        val devName = ad.descriptor.name.toUpperCase
        sb.append(s"\n\tpublic static final int INT_${devName} = $intIdx;")
        intIdx += ic
      }
    }
    sb.append("\n")
    ...
    val path = "java/apps/AtariSupervisor/src/atari/IoAddr.java"
    val writer = new java.io.PrintWriter(path)
    writer.print(sb.toString)
    writer.close()
    println(s"[IoAddrGenerator] Wrote $path")
  }
}
